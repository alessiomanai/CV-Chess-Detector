...
% There are several pre-trained networks that have gained popularity. Most
% of these have been trained on the ImageNet dataset, which has 1000 object
% categories and 1.2 million training images[1]. "AlexNet" is one such
% model and can be loaded using the |alexnet| function from Neural Network
% Toolbox(TM). Using |alexnet| requires that you first install 
% <matlab:helpview('nnet','alexnet') Neural Network Toolbox(TM) Model _for AlexNet Network_>.

% Load pre-trained AlexNet
net = alexnet()


% Other popular networks trained on ImageNet include VGG-16 and VGG-19 [3],
% which can be loaded using |vgg16| and |vgg19| from the Neural Network
% Toolbox(TM).


% |net.Layers| defines the architecture of the CNN. 
...
% As mentioned above, |net| can only process RGB images that are
...
% visualizing features from deeper layer weights can be done using
% |deepDreamImage| from Neural Network Toolbox(TM).
...
% features into a richer image representation [4].
%
...
% is named 'fc7'. Let's extract training features using that layer.
featureLayer = 'fc7';
...
newImage = fullfile('detectionnew/', '14.jpg');
